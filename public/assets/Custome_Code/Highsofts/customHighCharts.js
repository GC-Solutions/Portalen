...
function getTableData(table, columnAxisArray, graph_id, series_graph, graphValue, graphData , xAxisDS) {
    ...
    if(graphValue.graph_type == 11){
        chartXaxis = graphValue.graph_labels.XAxisData;
        ...
        var myTable = $('#Table_1').DataTable();
        var TableData = myTable.row().data();
        
        var ColumnIds = []; 
        var ColumnNames = graphValue.graph_labels.YAxisColumns;
        
        $.each(TableData , function(key, value){
            var title = myTable.column(key).header();
            var tempTitle = $(title).text();
            $.each(ColumnNames , function(colkey, colvalue){
                if(colvalue.trim() == tempTitle.trim()){
                    ColumnIds.push(key);
                    return false;
        ...
        $.each(ColumnIds , function(colkey, colvalue){
            var newCustomFormaula =  graphValue.graph_labels.custom_sumY;
            customFormula = newCustomFormaula;
            //console.log(newCustomFormaula);
            var lencount = 0;     
            $('#Table_1 tbody tr').each(function() {
                ...
                if($(this).attr("class") == 'dtrg-group dtrg-start dtrg-level-0'  || $(this).attr("class") == 'collapsed dtrg-group dtrg-start dtrg-level-0'){
                    ...
                    var goupHtml = Child[0]['innerHTML'];
                    var MainValue = Child[colvalue]['innerHTML'];
            ...
            if((customFormula != '' && customFormula != undefined ) || customFormula == 0){
                ...
                chartYaxistemp.push(customFormula);
                
            }

        });
        if (typeof chartYaxistemp !== 'undefined') {
           
            chartYaxis[0] = chartYaxistemp[0];
            totalAxis[1]  = chartYaxistemp[0];
            chartYaxisSecondary[0] = chartYaxistemp[1];
            totalAxis[2]  = chartYaxistemp[1];
            chartYaxisThird[0] = chartYaxistemp[2];
            totalAxis[3]  = chartYaxistemp[2];
        }
        //console.log(chartYaxis);
      
        dataArray.push(chartXaxis,chartYaxis,chartYaxisSecondary,chartYaxisThird,[], [], [], [],  totalAxis, [],[], [],  [], [], [] , [], [] , [] , [], [] );
        return dataArray;
    }else{
        if((table)){
            table.rows({ search: "applied" }).every(function(array,index, i) {
                ...
                xAxisDrill3.push(data[columnAxisArray.X3]);
                xAxisDrill4.push(data[columnAxisArray.X4]);
                xAxisDrill5.push(data[columnAxisArray.X5]);
                seriesAxisArray.push(data[columnAxisArray.X]);
                seriesAxisArray.push(parseFloat(data[columnAxisArray.Y].replace(/[.\s]/g, '').replace(/[,]/g, '.') * 1 ));
    ...
    //console.log(xAxisDrill3);
    
    var countXaxis = {};
    var countYaxis = {};
    var countYaxisCustomTemp = {};
    var countYaxisCustom= {};
    ...
    var countYaxisSecondaryCustomTemp = {};
    var countYaxisSecondaryCustom= {};
    ...
    $.each(countXaxis, function(keyXaxis, valueXaxis)
    {
        ...
        $.each(dataTableArray, function(key, value)
        {
            ...
            if(value[columnAxisArray.X] == keyXaxis && keyXaxis !== undefined){
                ...
                if(value[columnAxisArray.Y] != '' && value[columnAxisArray.Y] != undefined ){
                    ...
                    if(graphValue.graph_labels.custom_sumY != undefined)
                    {
                        var customSumString = graphValue.graph_labels.custom_sumY;
                        var customSumStringTotal = graphValue.graph_labels.custom_sumY;
                        re = /\(.*?\)/g;
                        var customSumY= customSumString.match(re);
                        $.each(customSumY , function(k , v){
                            var columnIndex= v.replace(/[{()}]/g, '');
                              
                            yAxisVal = parseFloat(value[columnIndex].replace(/[.\s]/g, '').replace(/[,]/g, '.') * 1) ;

                            if(resArr[columnIndex] != undefined){
                                resArr[columnIndex] = (yAxisVal||0) + resArr[columnIndex];
                            }
                            else{
                                resArr[columnIndex] = (yAxisVal||0);
                              
                            }
                            if(countYaxisCustom[columnIndex] != undefined){
                                countYaxisCustom[columnIndex] = (yAxisVal||0) + countYaxisCustom[columnIndex];
                            }
                            else{
                                countYaxisCustom[columnIndex] = (yAxisVal||0);
                            
                            }
                            
                            customSumString =  customSumString.replace('(' + columnIndex + ')', resArr[columnIndex]);
                            customSumStringTotal =  customSumStringTotal.replace('(' + columnIndex + ')', countYaxisCustom[columnIndex]);
                        ...
                        countYaxis[keyXaxis] =  customSumString;
                        countYaxisCustomTemp=  customSumStringTotal;
                       

                    }else{


                        yAxisVal =  parseFloat(value[columnAxisArray.Y].replace(/[.\s]/g, '').replace(/[,]/g, '.') * 1) ;
                        yAxisSum += (yAxisVal || 0);
                            ...
                            $.each(uniqueItems3 , function(itss, vass){
                                if(value[columnAxisArray.X3] == vass){
                                    ...
                                    if(yAxisSumDr3[vass] == undefined)
                                    {
                                        ...
                                        yAxisSumDr3[vass] = ((yAxisSumDr3[vass]+ yAxisValDr3) || 0); 
                                    }
                                    keyValue4X4 = vass;
                ...
                if(value[columnAxisArray.Y2] != '' && value[columnAxisArray.Y2] != undefined ){
                     ...
                     if(graphValue.graph_labels.custom_sumY2 != undefined)
                    {
                        var customSumString = graphValue.graph_labels.custom_sumY2;
                        var customSumStringTotal = graphValue.graph_labels.custom_sumY2;

                        re = /\(.*?\)/g;
                        var customSumY2= customSumString.match(re);
                       
                        $.each(customSumY2 , function(k , v){
                            var columnIndex= v.replace(/[{()}]/g, '');

                            yAxisSecondaryVal = parseInt(value[columnIndex].replace(/[.\s]/g, '').replace(/[,]/g, '.'));
                            if(resArr2[columnIndex] != undefined){
                                resArr2[columnIndex] = (yAxisSecondaryVal||0) + resArr2[columnIndex];
                            }
                            else{
                                resArr2[columnIndex] = (yAxisSecondaryVal||0);
                              
                            }
                            if(countYaxisSecondaryCustom[columnIndex] != undefined){
                                countYaxisSecondaryCustom[columnIndex] = (yAxisSecondaryVal||0) + countYaxisSecondaryCustom[columnIndex];
                            }
                            else{
                                countYaxisSecondaryCustom[columnIndex] = (yAxisSecondaryVal||0);
                            
                            }
                            
                            customSumString =  customSumString.replace('(' + columnIndex + ')', resArr2[columnIndex]);
                            customSumStringTotal =  customSumStringTotal.replace('(' + columnIndex + ')', countYaxisSecondaryCustom[columnIndex]);
                        ...
                        countYaxisSecondary[keyXaxis] =  customSumString;
                        countYaxisSecondaryCustomTemp=  customSumStringTotal;
                    

                    }else{
                        yAxisSecondaryVal = parseFloat(value[columnAxisArray.Y2].replace(/[.\s]/g, '').replace(/[,]/g, '.') * 1) ;
                ...
                if(value[columnAxisArray.Y3] == 0 || value[columnAxisArray.Y3] != '' && value[columnAxisArray.Y3] != undefined ){
                    ...
                    if(graphValue.graph_labels.custom_sumY3 != undefined)
                    {
                        ...
                        $.each(customSumY3 , function(k , v){
                            ...
                            yAxisThirdVal = parseFloat(value[columnIndex].replace(/[.\s]/g, '').replace(/[,]/g, '.'));
                            if(resArr3[columnIndex] != undefined){
                                resArr3[columnIndex] = (yAxisThirdVal||0) + resArr3[columnIndex];
                            }
                            else{
                                resArr3[columnIndex] = (yAxisThirdVal||0);
                            ...
                            customSumString =  customSumString.replace('(' + columnIndex + ')', resArr3[columnIndex]);
                        ...
                        yAxisThirdVal = parseFloat(value[columnAxisArray.Y3].replace(/[.\s]/g, '').replace(/[,]/g, '.') * 1) ;
                        yAxisThirdSum += (yAxisThirdVal || 0);
                        countYaxisThird[keyXaxis] =  yAxisThirdSum;

                    }
                }
                if(value[columnAxisArray.Y4] == 0 || value[columnAxisArray.Y4] != '' && value[columnAxisArray.Y4] != undefined ){
                    ...
                    if(graphValue.graph_labels.custom_sumY4 != undefined)
                    {
                        ...
                        $.each(customSumY4 , function(k , v){
                            ...
                            if(resArr4[columnIndex] != undefined){
                                resArr4[columnIndex] = (yAxisFourthVal||0) + resArr4[columnIndex];
                            }
                            else{
                                resArr4[columnIndex] = (yAxisFourthVal||0);
                            ...
                            customSumString =  customSumString.replace('(' + columnIndex + ')', resArr4[columnIndex]);
                        ...
                        yAxisFourthVal = parseFloat(value[columnAxisArray.Y4].replace(/[.\s]/g, '').replace(/[,]/g, '.') * 1) ;
                        yAxisFourthSum += (yAxisFourthVal || 0);
                        countYaxisFourth[keyXaxis] =  yAxisFourthSum;
                    }
                }
                if(value[columnAxisArray.Y5] == 0 || value[columnAxisArray.Y5] != '' && value[columnAxisArray.Y5] != undefined ){
                     if(graphValue.graph_labels.custom_sumY5 != undefined)
                    {
                        ...
                        $.each(customSumY5 , function(k , v){
                            ...
                            if(resArr5[columnIndex] != undefined){
                                resArr5[columnIndex] = (yAxisFifthVal||0) + resArr5[columnIndex];
                            }
                            else{
                                resArr5[columnIndex] = (yAxisFifthVal||0);
                            ...
                            customSumString =  customSumString.replace('(' + columnIndex + ')', resArr5[columnIndex]);
                        ...
                        yAxisFifthVal = parseFloat(value[columnAxisArray.Y5].replace(/[.\s]/g, '').replace(/[,]/g, '.') * 1) ;
                        yAxisFifthSum += (yAxisFifthVal || 0);
                        countYaxisFifth[keyXaxis] =  yAxisFifthSum;
                    }
                }
                if( value[columnAxisArray.Y6] == 0 || value[columnAxisArray.Y6] != '' && value[columnAxisArray.Y6] != undefined ){
                    ...
                    if(graphValue.graph_labels.custom_sumY6 != undefined)
                    {
                        ...
                        $.each(customSumY6 , function(k , v){
                            ...
                            if(resArr6[columnIndex] != undefined){
                                resArr6[columnIndex] = (yAxisSixthVal||0) + resArr6[columnIndex];
                            }
                            else{
                                resArr6[columnIndex] = (yAxisSixthVal||0);
                            ...
                            customSumString =  customSumString.replace('(' + columnIndex + ')', resArr6[columnIndex]);
                        ...
                        yAxisSixthVal = parseFloat(value[columnAxisArray.Y6].replace(/[.\s]/g, '').replace(/[,]/g, '.') * 1) ;
                ...
                if(value[columnAxisArray.Y7] == 0 || value[columnAxisArray.Y7] != '' && value[columnAxisArray.Y7] != undefined ){
                    ...
                    if(graphValue.graph_labels.custom_sumY7 != undefined)
                    {
                        ...
                        $.each(customSumY7 , function(k , v){
                            ...
                            if(resArr7[columnIndex] != undefined){
                                resArr7[columnIndex] = (yAxisSeventhVal||0) + resArr7[columnIndex];
                            }
                            else{
                                resArr7[columnIndex] = (yAxisSeventhVal||0);
                            ...
                            customSumString =  customSumString.replace('(' + columnIndex + ')', resArr7[columnIndex]);
                ...
                if(value[columnAxisArray.Y8] == 0 || value[columnAxisArray.Y8] != '' && value[columnAxisArray.Y8] != undefined ){
                    ...
                    if(graphValue.graph_labels.custom_sumY8 != undefined)
                    {
                        ...
                        $.each(customSumY8 , function(k , v){
                            ...
                            if(resArr8[columnIndex] != undefined){
                                resArr8[columnIndex] = (yAxisEigthVal||0) + resArr8[columnIndex];
                            }
                            else{
                                resArr8[columnIndex] = (yAxisEigthVal||0);
                            ...
                            customSumString =  customSumString.replace('(' + columnIndex + ')', resArr8[columnIndex]);
                        ...
                        yAxisEightVal = parseFloat(value[columnAxisArray.Y8].replace(/[.\s]/g, '').replace(/[,]/g, '.') * 1) ;
                        yAxisEightSum += (yAxisEightVal || 0);
                        countYaxisEight[keyXaxis] =  yAxisEightSum;
                    }
                }
                if(value[columnAxisArray.Y9] == 0 || value[columnAxisArray.Y9] != '' && value[columnAxisArray.Y9] != undefined ){
                    ...
                    if(graphValue.graph_labels.custom_sumY9 != undefined)
                    {
                        ...
                        $.each(customSumY9 , function(k , v){
                            ...
                            if(resArr9[columnIndex] != undefined){
                                resArr9[columnIndex] = (yAxisNinthVal||0) + resArr9[columnIndex];
                            }
                            else{
                                resArr9[columnIndex] = (yAxisNinthVal||0);
                            ...
                            customSumString =  customSumString.replace('(' + columnIndex + ')', resArr9[columnIndex]);
                        ...
                        yAxisNinthVal = parseFloat(value[columnAxisArray.Y9].replace(/[.\s]/g, '').replace(/[,]/g, '.') * 1) ;
        ...
        if(countYaxisCustomTemp == undefined || JSON.stringify(countYaxisCustomTemp) === '{}' ){
            
            totalAxis[1] = yAxisTotal = yAxisTotal + countYaxis[keyXaxis];
           
            countYaxis[keyXaxis] = parseFloat(countYaxis[keyXaxis].toFixed(0));
        }else{
               
                totalAxis[1] = countYaxisCustomTemp;
        }

        
        if(countYaxisSecondaryCustomTemp == undefined || JSON.stringify(countYaxisSecondaryCustomTemp) === '{}'){
           
            totalAxis[2] = yAxisSecondaryTotal = yAxisSecondaryTotal + countYaxisSecondary[keyXaxis] ;
            if(Object.keys(countYaxisSecondary).length !== 0){
                 countYaxisSecondary[keyXaxis] = parseFloat(countYaxisSecondary[keyXaxis].toFixed(0));
            }
           
        }else{
                totalAxis[2] = countYaxisSecondaryCustomTemp;
        }
        ...
        if(countYaxisThirdCustomTemp == undefined || JSON.stringify(countYaxisThirdCustomTemp) === '{}'){
            ...
            if(Object.keys(countYaxisThird).length !== 0){
                countYaxisThird[keyXaxis] = parseFloat(countYaxisThird[keyXaxis].toFixed(0));
            }
        }else{
        ...
        if(countYaxisFourthCustomTemp == undefined || JSON.stringify(countYaxisFourthCustomTemp) === '{}'){
            totalAxis[4] = yAxisFourthTotal = yAxisFourthTotal + countYaxisFourth[keyXaxis];
            if(Object.keys(countYaxisFourth).length !== 0){
                countYaxisFourth[keyXaxis] = parseFloat(countYaxisFourth[keyXaxis].toFixed(0));
            }
        }else{
        ...
        if(countYaxisFifthCustomTemp == undefined || JSON.stringify(countYaxisFifthCustomTemp) === '{}'){
            ...
            if(Object.keys(countYaxisFifth).length !== 0){
                countYaxisFifth[keyXaxis] = parseFloat(countYaxisFifth[keyXaxis].toFixed(0));
            }
        }else{
        ...
        if(countYaxisSixthCustomTemp == undefined || JSON.stringify(countYaxisSixthCustomTemp) === '{}'){
            totalAxis[6] = yAxisSixthTotal = yAxisSixthTotal + countYaxisSixth[keyXaxis];
            if(Object.keys(countYaxisSixth).length !== 0){
                countYaxisSixth[keyXaxis] = parseFloat(countYaxisSixth[keyXaxis].toFixed(0));
            }
        }else{
        ...
        if(countYaxisSeventhCustomTemp == undefined || JSON.stringify(countYaxisSeventhCustomTemp) === '{}'){
            ...
            if(Object.keys(countYaxisSeventh).length !== 0){
                countYaxisSeventh[keyXaxis] = parseFloat(countYaxisSeventh[keyXaxis].toFixed(0));
            }
        }else{
        ...
        if(countYaxisEightCustomTemp == undefined || JSON.stringify(countYaxisEightCustomTemp) === '{}'){
            ...
            if(Object.keys(countYaxisEight).length !== 0){
                 countYaxisEight[keyXaxis] = parseFloat(countYaxisEight[keyXaxis].toFixed(0));
            }
        }else{
        ...
        if(countYaxisNinthCustomTemp == undefined || JSON.stringify(countYaxisNinthCustomTemp) === '{}'){
            ...
            if(Object.keys(countYaxisNinth).length !== 0){
                countYaxisNinth[keyXaxis] = parseFloat(countYaxisNinth[keyXaxis].toFixed(0));
            }
        }else{
    ...
    //console.log("nkjnkjnkjnjknkjn");
    ////console.log( countYaxisThird);
    ...
    // }
        ...
        Object.keys(countYaxis).forEach(function(key) {
            ...
            if(columnAxisArray.X1 == undefined ){
                ...
                if(columnAxisArray.Y3 != undefined){
                    if(countYaxisThird[key] == undefined)
                    {
                        countYaxisThird[key] = 0;
                    }
                    chartYaxisThird.push(countYaxisThird[key])
                }
                ...
                chartYaxisEight.push(countYaxisEight[key])
    ...
    if (dataArray.length !== 0) {
        ...
        (uniqueLast).forEach(function (category) {var tempSum = 0;
            ...
            (uniqueFirst).forEach(function (year) {
                ...
                var sum1 = parseFloat(sum.toFixed(0));
                objs.push(sum1);
                tempSum = tempSum + sum;
                if (category === '01') {            
...
function createHighcharts(graphData, graphId, graphLabel,dataTable, columnAxisArray) {
    ...
    var columnSeriesEight = graphLabel.graph_labels.typeY8 ? graphLabel.graph_labels.typeY8 : "column";
    ...
    if (graphData != false && graphData != null) {
            ...
            if(i == 0)
            {
                ...
                for( var j in secondStep){
                    ...
                    for( var k in thirdStep){
                        ...
                        data123.push(objectcc);
                        ////console.log(yDrillDown3);
                        if(Object.keys(yDrillDown3).lengthy != 0){
                            ...
                            for( var l in fourthStep){
                                ...
                                data123.push(objectcc);
                                if(Object.keys(yDrillDown4).lengthy != 0){
        ...
        if(yAxisData != undefined && yAxisData[0] != undefined && yAxisData.length != 0){
                
                yAxisDataCount = totalGraph[1].toFixed(0).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');                    
        }else if(totalGraph[1] != undefined){
               yAxisDataCount = totalGraph[1].toFixed(0).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');  
        }
        //console.log(yAxisDataCount);
        testvar[lab] = yAxisDataCount;

           
        var lab = graphLabel.graph_labels.Y2;
      
        if(y2AxisData != undefined && y2AxisData.length != 0 && y2AxisData[0] != undefined && typeof totalGraph[2] !== 'object'){
                y2AxisDataCount = totalGraph[2].toFixed(0).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');                    
        }else if(totalGraph[2] != undefined){
                y2AxisDataCount = totalGraph[2].toFixed(0).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');   
         }
        testvar[lab] = y2AxisDataCount;

        var lab = graphLabel.graph_labels.Y3;
        if(y3AxisData != undefined  &&  y3AxisData[0] != undefined && y3AxisData.length != 0 ){
                y3AxisDataCount = totalGraph[3].toFixed(0).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');                    
        }
        ...
        if(y4AxisData.length != 0){
                y4AxisDataCount = totalGraph[4].toFixed(0).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');                    
        }
        testvar[lab] = y4AxisDataCount;

        var lab = graphLabel.graph_labels.Y5;
         if(y5AxisData.length != 0){
                y5AxisDataCount = totalGraph[5].toFixed(0).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');                    
        }
        ...
        var lab = graphLabel.graph_labels.Y6;
         if(y6AxisData.length != 0){
                y6AxisDataCount = totalGraph[6].toFixed(0).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');                    
        }
        testvar[lab] = y6AxisDataCount;
     
        var lab = graphLabel.graph_labels.Y7;
        if(y7AxisData.length != 0){
                y7AxisDataCount = totalGraph[7].toFixed(0).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');                    
        }
        testvar[lab] = y7AxisDataCount;

        var lab = graphLabel.graph_labels.Y8;
        if(y8AxisData.length != 0){
                y8AxisDataCount = totalGraph[8].toFixed(0).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');                    
        }
        testvar[lab] = y8AxisDataCount;

        var lab = graphLabel.graph_labels.Y9;
        if(y9AxisData.length != 0){
                y9AxisDataCount = totalGraph[9].toFixed(0).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');                    
        }
        ...
        if(graphLabel.graph_labels.is_series == true) {
            if (graphData[5] != false && graphData[5] != null) {
                ...
                $.each(graphData[5], function(key, value)
                {
                    ...
                    if (graphData != false && graphData != null) {
                        ...
                        testvar[tempName] = totalVal.toFixed(0).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');
                        series.push({
                            name: columnName,
            ...
            if(graphLabel.graph_type == 8)
            {
                ...
                $.each(xAxisData , function(key , value){
                    ...
                    $.each(yDrillDown[value], function(k , v) {
                        ...
                        newArrY.push(v[1]);
						// new part Code 
						var tempArr = {name : v[0] , y: v[1] , drilldown : graphLabel.graph_labels.Y+'|'+value+'|'+v[0]};
						var lab = graphLabel.graph_labels.Y+'|'+value+'|'+v[0];
                    	testvar[lab] =  v[1].toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');
						newArrYTest.push(tempArr);
						testArrForX1[graphLabel.graph_labels.Y+'|'+value+'|'+v[0]] = [v[0]];
						// Till here 
                    ...
                    $.each(y2DrillDown[value], function(k , v) {
                        ...
                        newArrY2.push(v);
						var tempArr2 = {name : v[0] , y: v[1] , drilldown : graphLabel.graph_labels.Y2+'|'+value+'|'+v[0]};
						var lab = graphLabel.graph_labels.Y2+'|'+value+'|'+v[0];
                    	testvar[lab] = v[1].toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');
						newArrY2Test.push(tempArr2);
						testArrForX1[graphLabel.graph_labels.Y2+'|'+value+'|'+v[0]] = [v[0]];
                        tempArrKey.push(v[0]);
					...
					$.each(newArrYTest, function(k1 , v1) {
						seriesDrill.push({
							name :v1['name'] ,
							id:v1['drilldown'],
							data:[v1['y']]
					...
					$.each(newArrY2Test, function(k1 , v1) {
						seriesDrill.push({
							name :v1['name'] ,
							id:v1['drilldown'],
							data:[v1['y']]
                ...
                if( typeof y2AxisData != "undefined" && typeof y2AxisData[0] != "undefined"){ 
                 series.push({
            ...
            if( typeof y3AxisData != "undefined" && y3Flag == 0 && typeof y3AxisData[0] != "undefined"){ 
            ...
            if(y6Flag == 0 && typeof y6AxisData != "undefined"){ 
                if(y6AxisData[0] == undefined){
                    y6AxisData[0] = 0;
                }
                series.push({
...
if(graphLabel.graph_type != '10'){
      (function(H) {
    H.wrap(H.Axis.prototype, 'drawCrosshair', function(proceed) {
      var axis = this;
      wasRendered = !!axis.cross; // on init, cross is not defiend
            ...
            events: {
                ...
                click:function(e)
                {
                   ...
                   if(e.target.nodeName != 'tspan'){
                    ...
                    if(graphLabel.graph_labels.is_series){
						
                        var searchVariable = e.target.textContent.split(':');
                        var tableId = dataTable.table().node().id;
                           
                        if(searchVariable[0] != ''){
                            var ColumnName = dataTable.columns(columnAxisArray.Y2).header()[0].id;
                         
                            var val = $('#exc_div' +tableId+ColumnName).attr("style");
                            if(val == 'display: none;'){
                                $('#mainS_' +tableId+ColumnName).hide();
                                $('#fromTo_' +tableId+ColumnName).hide();
                                $('#exc_div' +tableId+ColumnName).show();
                                $('#searchDiv_' +tableId+ColumnName).hide();
                            }
                            var mainValue = '';
                            var excVariable = $('#exc_div'+tableId+ColumnName).find('input:text').val();
                            if(excVariable)
                            {
                                if(excVariable.indexOf(searchVariable[0]+'|') >= 0)
                                {
                                    mainValue=excVariable.replace(searchVariable[0]+'|','');
                                    $('#exc_div'+tableId+ColumnName).find('input:text').val(mainValue);
                            
                                }else if(excVariable.indexOf(searchVariable[0]) > -1)
                                {
                                    mainValue = excVariable.replace(searchVariable[0],'');
                                    mainValue = excVariable.replace(/^\|+|\|+$/g, '');
                                    $('#exc_div'+tableId+ColumnName).find('input:text').val(mainValue);
                            
                                }else{
                                    mainValue=excVariable+'|'+searchVariable[0];
                                    $('#exc_div'+tableId+ColumnName).find('input:text').val(mainValue);
                                }
                            }else{
                                mainValue = searchVariable[0];
                                $('#exc_div'+tableId+ColumnName).find('input:text').val(mainValue);
                            }
                            if(searchVariable[0] != ''){
                                keyword = '^(?!.*('+mainValue+')).*$';
                            }
                            colId = columnAxisArray.Y2;
                        }else{
                            var ColumnName = dataTable.columns(columnAxisArray.X).header()[0].id;
                         
                            var keyword = xAxisData[Math.round(e.xAxis[0].value)];
                            var title = dataTable.columns(columnAxisArray.X).header();
                            title = $(title).html().replace(/[\W]/g, '-');
                            title = title + tableId;

                            var val = $('#exc_div' + tableId+ColumnName).attr("style");
                            if(val != 'display: none;'){
                                $('#exc_div'+tableId+ColumnName).find('input:text').val(keyword);
                            }
                            var val = $('#fromTo_' + tableId+ColumnName).attr("style");
                            if(val != 'display:none'){
                                $('#fromTo_'+tableId+ColumnName).find('input:text').val(keyword);
                            }
                            var val = $('#mainS_' + tableId+ColumnName).attr("style");

                            if(val != 'display: none;'){
                                $('#mainS_'+tableId+ColumnName).find('input:text').val(keyword);
                            }
                
                            colId = columnAxisArray.X;
                        ...
                        dataTable
                                ...
                                .draw( );
                        drawChart = false;
                        $('#'+title).val(keyword).trigger('change.select2');
                ...
                drilldown: function (e) {
                    ...
                    var testVar = e.type;
                   	    ...
                   	    var XAxisNumber = e.seriesOptions.id.split("|").length - 1;
					
                        if(e.seriesOptions.id.indexOf(graphLabel.graph_labels.Y)  >= 0 )
                        {
                            var valueForSearch = e.seriesOptions.id
                            var keyCheck = e.seriesOptions.id.split('|');
							
							if(XAxisNumber == 2){
								 valueForSearch = keyCheck[2].replace(graphLabel.graph_labels.Y,'');
							}else  {
								 valueForSearch = keyCheck[1].replace(graphLabel.graph_labels.Y,'');
							}
                           
                        }else if(e.seriesOptions.id.indexOf(graphLabel.graph_labels.Y2) >= 0)
                        {
                            var valueForSearch = e.seriesOptions.id
                            var keyCheck = e.seriesOptions.id.split('|');
							if(XAxisNumber == 2){
								 valueForSearch = keyCheck[2].replace(graphLabel.graph_labels.Y2,'');
							}else  {
								 valueForSearch = keyCheck[1].replace(graphLabel.graph_labels.Y2,'');
							}
                           
                        }
                        e.seriesOptions.name = e.seriesOptions.id;
                 		
                        this.xAxis[0].setCategories(Array.from(new Set(testArrForX1[e.seriesOptions.id])).sort(), false , false);
                       
						var COlAxisArr = columnAxisArray.X;
						if(XAxisNumber == 2){
							COlAxisArr = columnAxisArray.X1;
						}
					
                        var tableId = dataTable.table().node().id;
                        var ColumnName = dataTable.columns(COlAxisArr).header()[0].id;
                        var title = dataTable.columns(COlAxisArr).header();
                        title = $(title).html().replace(/[\W]/g, '-');
                        title = title + tableId;
                        $('#fromTo_'+tableId+ColumnName).find('input:text').val(valueForSearch);
                        $('#mainS_'+tableId+ColumnName).find('input:text').val(valueForSearch);
                        $('#exc_div'+tableId+ColumnName).find('input:text').val(valueForSearch);
                        
                        drawChart = true;
                        dataTable
                                .column(COlAxisArr)
                                .search(valueForSearch, true, false )
                                .draw( );
                        drawChart = true;
                        $('#'+title).val(valueForSearch).trigger('change.select2');
   

                },
                drillup: function (e) {
                    
                    //this.xAxis[0].setCategories(xAxisData, false , false);
				    var COlAxisArr = columnAxisArray.X;
 ...
 // },
  //TEST END
        ...
        xAxis: [
            {
              ...
              clickOnCrosshair: function(e, p) {
                ...
                $('#fromTo_'+tableId+ColumnName).find('input:text').val(p.category);
                $('#mainS_'+tableId+ColumnName).find('input:text').val(p.category);
                $('#exc_div'+tableId+ColumnName).find('input:text').val(p.category);
                
                $('#'+title).val(p.category).trigger('change.select2'); 
                drawChart = false;
                dataTable
                         .column(columnAxisArray.X)
                         .search(p.category, true, false )
                         .draw( );
                drawChart = false;
                //$('#'+title).select2('destroy');
                //$('#'+title).val(p.category).select2();
        ...
        yAxis: [
            ...
            {
                title: {
                    text: graphLabel.graph_labels.Y
                },
                min: yAxismin ,
                max: yAxismax
            },
            {
                title: {
                    text: graphLabel.graph_labels.Y2
                },
                min: y2Axismin ,
                max: y2Axismax,
                opposite: true,
                visible: isSecondaryY2axis
    ...
    hover: {
      ...
      marker: {
       ...
       //  symbol: null           // A predefined shape or symbol for the marker. When null, the symbol is pulled from options.symbols. Other possible values are "circle", "square", "diamond", "triangle" and "triangle-down".
        ...
        legend: {
            backgroundColor: "#FFFFFF", // change that one 
            shadow: true
        },
...
series:{
       ...
       events: {
                ...
                legendItemClick: function (event) {
                    ...
                    var searchVariable = this.name;
                    var tableId = dataTable.table().node().id;
                    var flag = 0 ;
                    var ColumnName = dataTable.columns().header();
                    
                    $.each(Object.keys(ColumnName) , function(k , v){
                        
                        var num = $.isNumeric(parseInt(v));
                        
                        if(num)
                        {
                            if(ColumnName[parseInt(v)].id == searchVariable){
                                 flag = 1;
                                return false ;
                            }
                    ...
                    if(flag == 0)
                    {
                        var title1 = '';
                        var ColumnName = dataTable.columns(columnAxisArray.Y2).header()[0].id;
                        var title = dataTable.columns(columnAxisArray.Y2).header();
                        ...
                        if(excVariable)
                        {
                            if(excVariable.indexOf(searchVariable+'|') >= 0)
                            {
                                mainValue=excVariable.replace(searchVariable+'|','');
                                $('#exc_div'+tableId+ColumnName).find('input:text').val(mainValue);
                        
                            }else if(excVariable.indexOf(searchVariable) > -1)
                            {
                                mainValue = excVariable.replace(searchVariable,'');
                                mainValue = excVariable.replace(/^\|+|\|+$/g, '');
                                $('#exc_div'+tableId+ColumnName).find('input:text').val(mainValue);
                                ...
                                mainValue=excVariable+'|'+searchVariable;
                                $('#exc_div'+tableId+ColumnName).find('input:text').val(mainValue);
                            }
                        }else{
                            mainValue = searchVariable;
                            $('#exc_div'+tableId+ColumnName).find('input:text').val(mainValue);
                        }
                        if(searchVariable){
                            keyword = '^(?!.*('+mainValue+')).*$';
                        ...
                        dataTable
                                ...
                                .draw( );
                        drawChart = false;
                        $('#'+title1).val(keyword).trigger('change.select2');
                    }
       ...
       point: {
             events: {
                click: function(event) {
                    ...
                    if(graphLabel.graph_labels.is_series){
                        var searchVariable = this.series.name;
                      
                        var tableId = dataTable.table().node().id;
                        var ColumnName = dataTable.columns(columnAxisArray.Y2).header()[0].id;
                        var y2Axis = columnAxisArray.y2;
                        var title = dataTable.columns(y2Axis).header();
                        ...
                        var searchVariable = this.category;
                        var ColName = '';
                        var tableId = dataTable.table().node().id;
						if(columnAxisArray.X1 != undefined){
							var ColumnName = dataTable.columns(columnAxisArray.X1).header()[0].id;
					    	var title = dataTable.columns(columnAxisArray.X1).header();
							ColName = columnAxisArray.X1;
						}else {
							var ColumnName = dataTable.columns(columnAxisArray.X).header()[0].id;
					    	var title = dataTable.columns(columnAxisArray.X).header();
							ColName = columnAxisArray.X;
						}
                        ...
                        dataTable
                                ...
                                .draw( );
                        drawChart = false;
                        $('#'+title).val(searchVariable).trigger('change.select2'); 
...
//This is the dsign of teh sidbar for highcharts.
/*legend:{
       layout: 'vertical',
        align: 'center',
        verticalAlign: 'top',
        ...
        itemStyle: {
         ...
         legend: {
            ...
            labelFormatter: function() {
                ...
                var nameL = this.name.trim();
               
                return '<span style="color:' + this.color + '">' + this.name + ': (' + testvar[nameL] + ')</span>';
            },
