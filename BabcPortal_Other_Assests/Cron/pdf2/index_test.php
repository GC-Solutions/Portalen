...
for($j = 2; $j < count($file); $j++){ 
    ...
    if((!in_array(trim($mailFileName) , $AllFileName))){
        ...
        for ($i= 0; $i < $number ; $i++) { 
            ...
            if($i == 0){
              ...
              //  var_dump($column);
                ...
                $column = str_replace("==SÄCK", " SÄCK", $column);
            ...
            // $column  =str_replace(" 16597,31", "", $column); 
                ...
                foreach(  $tempcol as $tKey => $tVal){
                    ...
                    if($key >= 0 && array_search($tVal ,$column) !== false ){
                        ...
                        if((strpos($tVal , 'TOTAL EXKL MOMS') !== false )){
                        ...
                        if(strpos($tVal ,  ' SÄCK') !== false){
                            ...
                            if(end($KRT) != 'SÄCK' ){
                                ...
                                $KRT = str_replace(" SÄCK ", " SÄCK==", $KRTVAl); 
                                $column[$key] = $KRT;
       ...
       foreach ($column as $item) {
        ...
        if($item == "532192") {
         //   echo "</br>".$column[$b-1]."</br>".$item."</br>".$column[$b+1]."</br>".$column[$b+2]."</br>".$column[$b+3]."</br>".$column[$b+4]."</br>".$column[$b+5]."</br>".$column[$b+6];
        }




        if (strpos($column[$b], ':') === false && strpos($column[$b+1], ':') === false && strpos($column[$b+2], ':') === false && strpos($column[$b+3], ':') === false && strlen(($item))<=12 && strlen($item)>=9 && strlen(trim($column[$b+1]))<=12 && isset($column[$b + 1]) && !empty($column[$b + 1]) && strlen(trim($column[$b+2]))<=13&&isset($column[$b + 2]) && !empty($column[$b + 2]) && strlen(trim($column[$b+3]))<=12 && isset($column[$b + 3]) && !empty($column[$b + 3])) {
          //  echo "</br>".$column[$b]."</br>".$column[$b+1]."</br>".$column[$b+2]."</br>".$column[$b+3]."</br>".$column[$b+4];
            $concatenatedString = ' ' . $item;
        } 
        elseif (strpos($column[$b], ':') === false && strpos($column[$b+1], ':') === false && strpos($column[$b+2], ':') === false && strpos($column[$b-1], ':') === false &&  strlen(($item))<=12 && strlen(trim($column[$b-1]))<=12 && strlen(trim($column[$b+1]))<=12 && isset($column[$b + 1]) && !empty($column[$b + 1]) && strlen(trim($column[$b+2]))<=12 && isset($column[$b + 2]) && !empty($column[$b + 2])) {

            $concatenatedString = $concatenatedString.' ' . $item;
        }
        elseif (strpos($column[$b], ':') === false && strpos($column[$b+1], ':') === false && strpos($column[$b-2], ':') === false && strpos($column[$b-1], ':') === false &&  strlen(($item))<=12 && strlen(trim($column[$b-2]))<=12 && strlen(trim($column[$b-1]))<=12 && (strlen(trim($column[$b+1]))<=12)&&isset($column[$b + 1]) && !empty($column[$b + 1])) {

            $concatenatedString = $concatenatedString.' '  . $item;
        }
        elseif (strpos($column[$b], ':') === false && strpos($column[$b-1], ':') === false && strpos($column[$b-2], ':') === false && strpos($column[$b-3], ':') === false &&  strlen(trim($item))<=12 && strlen(trim($column[$b-3]))<=12 && strlen(trim($column[$b-2]))<=12 && strlen(trim($column[$b-1]))<=12) {

            $concatenatedString = $concatenatedString.' '  . $item;
            array_push($newArray, $concatenatedString);
     
        }
        elseif(strpos($column[$b], ':') === false && strlen($item)<=8 && strlen($column[$b-1])>=20 && strlen($column[$b+1])<=8) {
            $concatenatedString = $item . '    ' .$column[$b+1];
            array_push($newArray, $concatenatedString);
            $concatenatedString = "";
            ...
            if(strpos($column[$b], ':') === false && strpos($column[$b-1], ':') === false && strpos($column[$b], 'MARKNADSRABATT') === false) {
                          ...
                          if(count($checkVal)>1) {
                            ...
                            if($lineCnt == 3)
                            {
                                ...
                                if(count($checktestValArr) <= 2 && count($checktestValArr) > 0 ){
                                    ...
                                    VALUES (:Fakturanr,:Fakt_datum,:Sida,:Varumottagare,:supplier,:SUPPORT_SALES_APPLY_PERIOD,
                                    :Kundnr,:Kundnman,:Avtal,:NAME,:ENHET,:ARTIKEL,:LEV_ARTN_UTAN_SK,:KVANT,:A_PRIS_BRUTTO,:A_PRIS_NETTO,:RADNETTO,:FileNamePDF)";
                                if($BENÄMNING=="MARKNADSRABATT") {
                                    $BENÄMNING  =  $BENÄMNING . $val;
                                    echo $BENÄMNING;
                                    var_dump($checkVal);
                                }
                                echo $BENÄMNING." ".$val." 2st";
                                $stmt = $db->prepare($sql);
...
echo $BENÄMNING." 3st";
                            ...
                            if($lineCnt == 0){
                                ...
                                if(trim(end($checkVal)) == 'KRT' || trim(end($checkVal)) == 'ST'|| trim(end($checkVal)) == 'SÄCK' || trim(end($checkVal)) == 'HINK' || trim(end($checkVal)) == 'KR' ){
                                    $ENHET = end($checkVal);
                                }
