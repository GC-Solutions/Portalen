            ...
            //if( $SVal['PageMenuText']=='Total orders'){
                ...
                // $selectedPlaceholderId = $PVal['PlaceholderId'];
                    ...
                    if($RedisCheckTTL === 1) {
                       ...
                       $currentTTL = $redis->ttl($keyNameTTL)."</br>";
                       $intCurrentTTL = intval($currentTTL);
                        if(($userTTL-$intCurrentTTL-600)>0) {
                            echo "</br>TTL Is still ok, exiting loop...";
                            continue;
                        }
							if($intCurrentTTL > ($userTTL-100)) {
						  echo "</br>Avoiding conflicts, exiting loop...";
                            continue;
						}
                    }
                    echo "\n".$placeholderId."\n";
                    echo $getPlaceholderDetails1[0]['AllowJsonSave']." ".$getPlaceholderDetails1[0]['AllowJsonSave']."\n";
                        ...
                        if(isset($_REQUEST['ticket_id']))
                        {
                            ...
                            if(isset($getPlaceholderDetails) && $getPlaceholderDetails[0]['ApiType'] == '2')
                            {
                                ...
                                if($getPlaceholderDetails[0]['EnableCacheTable']){
                                    $keyName = $_SESSION['CompanyID'] .'-'.$_SESSION['ParentUserID'].$_SESSION['username']  .'-'.$placeholderId;
