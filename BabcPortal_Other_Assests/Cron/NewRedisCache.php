...
//define('baseUrl','http://127.0.0.1/public/');
...
if (strpos($email, ',') !== false) {
    // The string contains a comma, so let's split it
    $delimiter = ",";
    $parts = explode($delimiter, $string);
    $email_tested = $parts[1];
...
echo $email_tested;
    ...
    //    $endTime = microtime(true);
      //  $executionTime = ($endTime - $startTime) * 1000;
        // echo "Execution Time, verifyGroupUser: " . number_format($executionTime, 2) . " ms</br>";
...
function getUserPages($userId){
	try {
        //$startTime = microtime(true);
          ...
          //  $endTime = microtime(true);
            //$executionTime = ($endTime - $startTime) * 1000;
            //echo "Execution Time, getUserPages: " . number_format($executionTime, 2) . " ms</br>";
 ...
 function getUserDetails($id){
    //$startTime = microtime(true);
      ...
      //      $endTime = microtime(true);
        //    $executionTime = ($endTime - $startTime) * 1000;
          //  echo "Execution Time, getUserDetails: " . number_format($executionTime, 2) . " ms</br>";
            return $data;
        }
...
function getCompanyDetails($id){
 //   $startTime = microtime(true);
   ...
   //     $endTime = microtime(true);
     //   $executionTime = ($endTime - $startTime) * 1000;
       // echo "Execution Time, getCompanyDetails: " . number_format($executionTime, 2) . " ms</br>";
        return $data;
 ...
 function getCompaniesDetails($id)
{
    ...
    //$endTime = microtime(true);
    //$executionTime = ($endTime - $startTime) * 1000;
    //echo "Execution Time, getCompaniesDetails: " . number_format($executionTime, 2) . " ms</br>";
    return $data;
}
 function getPagePlaceholders($pageId, $userId, $pageText)
{
   // $startTime = microtime(true);
    ...
    //$endTime = microtime(true);
    //$executionTime = ($endTime - $startTime) * 1000;
    //echo "Execution Time, getPlaceholders: " . number_format($executionTime, 2) . " ms</br>";
    return $data;
}
function getDatasourceTableDetails($id)
{
    ...
    try {
      ...
      //  $endTime = microtime(true);
        //$executionTime = ($endTime - $startTime) * 1000;
        //echo "Execution Time, getDataSourceTableDetails: " . number_format($executionTime, 2) . " ms</br>";
...
function getTableActionDetails($id)
{
    ...
    try {
      ...
      //  $endTime = microtime(true);
        //$executionTime = ($endTime - $startTime) * 1000;
        //echo "Execution Time, getTableActionDetails: " . number_format($executionTime, 2) . " ms</br>";
        return $data = $stmt->fetchAll();
    } catch (Exception $exc) {
    }
}
function getTableActionDetailsByIdIN($id){
    ...
    try {
      ...
      //  $endTime = microtime(true);
        //$executionTime = ($endTime - $startTime) * 1000;
        //echo "Execution Time, getTableActionDetailsByIdIN: " . number_format($executionTime, 2) . " ms</br>";
        return $data = $stmt->fetchAll();
    } catch (Exception $exc) {
    }
}
function executeQuery($query, $databaseName, $Table = '' ){
    ...
    try{
      ...
      //  $endTime = microtime(true);
       // $executionTime = ($endTime - $startTime) * 1000;
        //echo "Execution Time, executeQuery: " . number_format($executionTime, 2) . " ms</br>";
    return $data;
}
function ColumnProperties($columnData,$singleColumnValue)
{
    ...
    //$endTime = microtime(true);
    //$executionTime = ($endTime - $startTime) * 1000;
    //echo "Execution Time, columnProperties: " . number_format($executionTime, 2) . " ms</br>";
    return $columnData;
}
...
function columnDataRound($columnData,$singleColumnValue)
{
    ...
    //$endTime = microtime(true);
    //$executionTime = ($endTime - $startTime) * 1000;
    //echo "Execution Time, columnDataRound: " . number_format($executionTime, 2) . " ms</br>";
    return $columnData;
}
...
//    $startTime = microtime(true);
  ...
  //  $endTime = microtime(true);
    //$executionTime = ($endTime - $startTime) * 1000;
    //echo "Execution Time, searchArray: " . number_format($executionTime, 2) . " ms</br>";
    return false;
}
function addforLoops( $value , $params , $count = 0  , $cntNew = 0 )
    {
      //  $startTime = microtime(true);
        ...
        //$endTime = microtime(true);
       // $executionTime = ($endTime - $startTime) * 1000;
       // echo "Execution Time, addForLoops: " . number_format($executionTime, 2) . " ms</br>";
...
function SumCalulation($sumType , $customSumData , $columnSumResults , $getColumnsProperties , $sumColumnLable , $searchValue , $searchValueArray , $columnSumResultsValue , $value1 , $dataToTable  ,$type = 'multiple'){
    ...
    //$endTime = microtime(true);
   // $executionTime = ($endTime - $startTime) * 1000;
   // echo "Execution Time, sumCalculations: " . number_format($executionTime, 2) . " ms</br>";
}
 function getActionButton($tableActions , $dataToTable , $actationTableColumn , $placeholderId , $userPagePlaceholder){
    ...
    //$endTime = microtime(true);
    //$executionTime = ($endTime - $startTime) * 1000;
    //echo "Execution Time, getActionButtons: " . number_format($executionTime, 2) . " ms</br>";
...
{    //$startTime = microtime(true);
    ...
    //$endTime = microtime(true);
    //$executionTime = ($endTime - $startTime) * 1000;
    //echo "Execution Time, getPageText: " . number_format($executionTime, 2) . " ms</br>";
    return $data;
}
// Main Code 
$startTime = microtime(true);
	...
	foreach($Users as $UKey => $UValue){
		...
		if($getUserDetails && !empty($getUserDetails['UserGroupActiveFlag']) && !empty($getUserDetails['AvailableUserGroup'])  && empty($getUserDetails['DefaultLogin']) )
        {
            ...
            $getUserDetails = verifyGroupUser($userCount[1]);
        ...
        foreach($_SESSION['PageDetails'] as $Skey => $SVal){
            ...
            //if( $SVal['PageMenuText']=='Total orders'){
                ...
                // $selectedPlaceholderId = $PVal['PlaceholderId'];
                    ...
                    $keyNameTTL = $_SESSION['CompanyID'] .'-'.$_SESSION['ParentUserID'] .'-'.$placeholderId;
					
					  $isLocked =   $redis->exists($keyNameTTL."-Lock");
         			if($isLocked===1) {
					     echo $keyNameTTL." Key is locked, exiting loop...";
						$redis->del($keyNameTTL."-Lock");
                            continue;
					
					}
					
                    $userTTL = intval($getPlaceholderDetails1[0]['TimeDurationRedisTable'])*60;
                    if (isset($userTTL) && $userTTL !== 0) {
                                       
                    } else {
                    $userTTL = 600;
                    }
                    $RedisCheckTTL =  $redis->exists($keyNameTTL) ;

                    if($RedisCheckTTL === 1) {
                       
                       $currentTTL = $redis->ttl($keyNameTTL);
                       $intCurrentTTL = intval($currentTTL);
                        if(($userTTL-$intCurrentTTL-600)>0) {
                            echo $keyNameTTL." TTL Is still ok, exiting loop...";
                            continue;
                        }
						if($intCurrentTTL > ($userTTL-100)) {
						  echo $keyNameTTL." Avoiding conflicts, exiting loop...";
                            continue;
						}
                    }
                 //   echo "</br>".$placeholderId."</br>";
                   // echo $getPlaceholderDetails1[0]['AllowJsonSave']." ".$getPlaceholderDetails1[0]['AllowJsonSave']."</br>";
                    if(isset($getPlaceholderDetails1[0]['AllowJsonSave']) && $getPlaceholderDetails1[0]['AllowJsonSave'] == 1 ){
                       
                    //    echo "</br>AllowJsonIsSet";
                        $FilterSession = 0;
                       ...
                       // echo "(Start) processing for Action Button ";
                  ...
                  //      echo "(Start)if we have fetched data for placeholder";
                        if(count($getPlaceholderDetails1) >= 1)
                        {
                            ...
                            foreach ($getPlaceholderDetails1 as $ke => $valueee) {
                               ...
                               // echo "(Start) if their is any detail of placeholder ";
                                ...
                                if ($requestType == 3) {
                                  ...
                                  //  echo "(Start) For loop for data fetched .";
                             ...
                             //  echo "(Start) Fetching Data  from Data Source Call Api  ";
                                else {
                      ...
                      //  echo "Condition to be exceuted in case of Multiple Node .";
                        if(isset($_REQUEST['ticket_id']))
                        {
                            echo $_SESSION['CompanyID'] .'-'.$_SESSION['UserID'] .'-'.$placeholderId;
                            return $tableData;
                        }else{
                            if(isset($getPlaceholderDetails) && $getPlaceholderDetails[0]['ApiType'] == '2')
                            {
                                echo "redis1".$_SESSION['CompanyID'] .'-'.$_SESSION['UserID'] .'-'.$placeholderId;
                                $input = array();
                                ...
                                if($getPlaceholderDetails[0]['EnableCacheTable']){
                                  ...
                                  $isLocked =   $redis->exists($keyName."-Lock");
         								if($isLocked===1) {
					  				
											$redis->del($keyName."-Lock");
                         				   echo $keyName." Key is Locked";
					
											}
								   else {
									                       $RedisCheck =  $redis->exists([$keyName,$keyName]) ;                                 
                                    ...
                                    $TTL = 600;
                                        ...
                                        $redis->del($keyName);
								               ...
								               $redis->hset($keyName,$keyName, $NewData );
                                                        $redis->expire($keyName, $TTL);
                                                        echo $keyName." Key has been updated with a TTL: ".$TTL." Seconds</br>";
								   
								   }
                                }
                            
                            }else{
                                //echo "redis2".$_SESSION['CompanyID'] .'-'.$_SESSION['UserID'] .'-'.$placeholderId;
                                // if( is_dir($CompanyDir) === false )
                                // {
                                //     mkdir($CompanyDir , 0777);
                                // }
                                ...
                                //var_dump($getPlaceholderDetails);
                               if($getPlaceholderDetails[0]['EnableCacheTable']){
